...
public class Dados implements Serializable {
        ...
        for(Pessoa pessoa : pessoas){
            ...
            for(Voto voto : votos){
                ...
                if(voto.getEleicao().equals(eleicao)){
                    ...
                    String data = String.format("%d-%d-%d %d:%d",horas.get(Calendar.DATE),horas.get(Calendar.MONTH)+1,horas.get(Calendar.YEAR),horas.get(Calendar.HOUR),horas.get(Calendar.MINUTE));
    ...
    public synchronized String addVoto(int nEleicao, int nlista, int nCC, int mesa, GregorianCalendar time){
        Eleicao eleicao = eleicoesAtuais.get(nEleicao-1);
        MesaDeVoto mesaDeVoto = mesasDeVoto.get(mesa-1);
        ArrayList<Lista> listas = eleicao.getListas();
        if(listas.size()==0){
